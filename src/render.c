...
inline void	re_pixel_column(t_data *data, double *color, int x, int di)
{
	int		i;
	t_tex	buf;

	i = (data->mlx.win_hmid) - di;
	re_pixel_column_utils(color, &buf, data);
	if (i < 0)
		i = 0;
	while (i < (data->mlx.win_hmid) + di && i < data->mlx.win_h)
	{
		if ((color[0] == 1 || color[0] == 4) && i >= 0 && i < \
		data->mlx.win_h && color[1] * buf.wi > 0 && color[1] * buf.wi < buf.wi)
		{
			re_pixeltoimg(data->mlx.game, x, i, \
			re_take_img_pixel(buf.id, color[1] * buf.wi, ((float)(i - \
			((data->mlx.win_hmid) - di)) / (di * 2)) * buf.he));
		}
		if ((color[0] == 2 || color[0] == 3) && (i >= 0 && i < \
		data->mlx.win_h && color[1] * buf.wi > 0 && color[1] * buf.wi < buf.wi))
		{
			re_pixeltoimg(data->mlx.game, x, i, \
			re_take_img_pixel(buf.id, buf.wi - (color[1] * buf.wi), \
